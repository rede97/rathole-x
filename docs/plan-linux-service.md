...
> Windows 侧对应实现见 `src/platform/windows.rs`。

## 目标

`sudo rathole-x service install <server|client>` 在 Linux 上一键完成：写入 systemd unit、放置角色骨架配置、开机自启并立即启动。initd(SysV)明确不支持——发行版碎片化严重，现代发行版均为 systemd。

## CLI 语义(与 Windows 对齐)

```
sudo rathole-x service install <server|client> [--name <N>] [-c /etc/rathole-x/<N>.toml]
...
- 权限模型（已实现于 Windows 侧，Linux 实现期沿用）：
  - `version.toml`（与配置同目录）记录 `version = <大版本>`，由 `service install` 写入；无权限策略文件——CLI 以文件实际写权限（探测）决定是否提权，`service install --allow-user-config` 仅设置 ACL。
...
| 路径 | 说明 |
|---|---|
| `/etc/systemd/system/rathole-x-<role>-<name>.service` | unit 文件（一服务一 unit，单角色） |
...
[Service]
Type=simple
ExecStart=<current_exe> run --config /etc/rathole-x/<name>.toml
Restart=on-failure
RestartSec=3
LimitNOFILE=1048576
...
1. 新建 `src/platform/linux.rs`（与 `windows.rs`/`other.rs` 同层，经 `platform` facade 暴露）,`#[cfg(target_os = "linux")]`,API 与 Windows 侧对称：
   `install(SystemdInstallOptions) -> Result<()>` / `uninstall(service_name) -> Result<()>`。
2. lib.rs 的 `service install/uninstall` 分发：`cfg(linux)` 分支替换当前的 "planned" 提示。
3. systemctl 调用用 `std::process::Command`(`daemon-reload`、`enable --now`),不引入 D-Bus 依赖;命令失败原样透传 stderr。
4. `service uninstall`:`disable --now`(best-effort)→ 删 unit → `daemon-reload`。
5. 验收：在 Linux 机器或容器实测 `service install` 后 `systemctl status` active、重启后自启、`service uninstall` 无残留。

## 风险

- WSL/容器无 systemd → `service install` 检测 `/run/systemd/system` 不存在则报错说明。
