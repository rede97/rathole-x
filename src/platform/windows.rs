...
/// execute the binary, read the config and write its own logs. With
/// `allow_user_config` the directory additionally grants Users create-files
/// and the config Users modify — never read-for-everyone.
...
mod tests {
    ...
    fn lockdown_args_restrict_dir_exe_and_config() {
        ...
        assert_eq!(
            cmd_string(&cmds[1]),
            "/inheritance:r /grant *S-1-5-32-544:(OI)(CI)F /grant *S-1-5-18:(OI)(CI)F /grant NT SERVICE\\rathole-x-server-default:(OI)(CI)RX"
        );
        ...
        // Config: SYSTEM and the service SID only need read; no Users
        // access by default.
        assert_eq!(
            cmd_string(&cmds[5]),
            "/inheritance:r /grant *S-1-5-32-544:F /grant *S-1-5-18:R /grant NT SERVICE\\rathole-x-server-default:(R)"
        );
    ...
    fn lockdown_args_allow_user_config_grants_write_only() {
        ...
        assert_eq!(
            cmd_string(&cmds[1]),
            "/inheritance:r /grant *S-1-5-32-544:(OI)(CI)F /grant *S-1-5-18:(OI)(CI)F /grant NT SERVICE\\rathole-x-server-default:(OI)(CI)RX /grant *S-1-5-32-545:WD"
        );
        ...
        assert_eq!(
            cmd_string(&cmds[5]),
            "/inheritance:r /grant *S-1-5-32-544:F /grant *S-1-5-18:R /grant NT SERVICE\\rathole-x-server-default:(R) /grant *S-1-5-32-545:M"
        );
    }
    #[tokio::test]
