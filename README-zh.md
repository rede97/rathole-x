...
- **Windows 服务安装** — `service install server|client --name <n>` 注册命名的 AutoStart SCM 服务，运行身份是 `NT AUTHORITY\LocalService`，带**受限的每服务 SID**，仅保留 `SeChangeNotifyPrivilege`，绝不使用 LocalSystem。共享受保护二进制仅向各 `NT SERVICE\rathole-x-<role>-<n>` SID 授予读/执行；每服务使用独立受保护的日志目录，配置仅由其 SID 读取，Administrators 与 SYSTEM 保留生命周期控制。托管服务拒绝 `--allow-user-config`，因为原子配置替换无法安全地暴露共享配置目录；普通卸载会保留受保护的含密钥配置。
...
> **Linux systemd / OpenRC**：同样的命令须以 `sudo` 运行（`sudo rathole-x service install server --yes --name relay`）。systemd 写入、启用并启动 `rathole-x-server-relay.service`；OpenRC（Alpine、Gentoo）写入 `/etc/init.d/rathole-x-server-relay`，并通过 `rc-update add default` 注册。两者都以专用、不可登录的 `rathole-x:rathole-x` 账号而不是 root 运行受保护的 root:root `0755` `/usr/local/lib/rathole-x/rathole-x`。`/etc/rathole-x` 为 root:rathole-x `0750`；配置和 `version.toml` 为 root:rathole-x `0640`，凭据不会暴露给 root 与服务组之外的用户。root 以原子方式管理配置并触发热重载；`--allow-user-config` 会明确拒绝，因为安全的原子替换必须修改目录。systemd 与 OpenRC 对低端口仅授予 `CAP_NET_BIND_SERVICE`；systemd 还限制 capability 并启用 `NoNewPrivileges`。OpenRC 的 root supervisor 持有 pid 和 `/var/log` 输出。普通卸载保留受保护的配置，也绝不会删除管理员预先创建的 `rathole-x` 账号。已安装服务只支持命名托管布局：`service install --config …` 会被拒绝，请改用 `--name`。Linux 不做自提权：非 root 运行会报 `please run with sudo`。见 [docs/plan-linux-service.md](docs/plan-linux-service.md)。
...
```bash
Windows 上，`status` 还会查询由规范化配置路径派生、受 ACL 保护的**仅本机、只读**命名管道。已认证的本地调用者最多请求一个有界快照，远程客户端被拒绝；端点只报告 schema 版本、角色、进程 ID、采集时间和连接状态，绝不暴露 token、密钥、载荷或流量。
...
- `service install <server|client> --yes [--name N] [--allow-user-config]` — 以受管 `<config-dir>/<N>.toml` 布局安装命名、单角色系统服务（为了让生命周期命令始终可发现它，`--config` 会被拒绝）。Windows 将二进制复制到配置旁并以 UAC 注册 SCM 服务；Linux 在 `sudo` 下部署 root 所有的 `/usr/local/lib/rathole-x/rathole-x`，并写入 systemd unit 或 OpenRC 脚本。`--name` 默认为 "default"；Windows 和 Linux 的受管服务都会拒绝 `--allow-user-config`，以便在原子替换期间保护配置目录。
    - [Logging](#logging)
    - [Tuning](#tuning)
  - [Benchmark](#benchmark)
